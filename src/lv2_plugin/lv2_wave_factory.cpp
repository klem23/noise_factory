...
#define WA_URI	"http://github.com/klem23/noise_factory/wave_factory";
...
int signal_change_handler(const char *path, const char *types, lo_arg **argv, int argc,
	...
	int size = lo_blob_datasize(argv[0]) / sizeof(float);
...
void cleanup(LV2_Handle instance){
	wave_handle* hdl = (wave_handle*)instance;
	delete hdl->gen;
	delete hdl->filter;
	delete[] hdl->buff;
...
{
	wave_handle* hdl = new wave_handle;
	wave_gen* wave = new wave_gen(s_rate);
	conv_filter* filter = new conv_filter(s_rate);
...
void run(LV2_Handle instance, uint32_t sample_count){
	...
	wave_handle* plugin = (wave_handle*)instance;

	if(plugin->buff == NULL){
		plugin->buff = new float[sample_count];
		plugin->gen->set_output(plugin->buff);
		plugin->filter->set_input(plugin->buff);
	}

	/*liblo check before running*/
	//      lo_server_recv_noblock(los, 0);

	plugin->gen->set_vol(vol);
	plugin->gen->process(sample_count);
	plugin->filter->process(sample_count);
	//	}
}
